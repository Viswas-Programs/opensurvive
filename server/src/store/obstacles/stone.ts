...
export default class Stone extends Obstacle {
	...
	damage(dmg: number) {
		super.damage(dmg);
		world.onceSounds.push({ path: `obstacles/stone_hit.mp3`, position: this.body.position });
	}
	...
	die() {
		...
		switch (this.special) {
			case "ak47": {
				...
				if (ak47)
					spawnGun(ak47.nameId, ak47.color, this.body.position, ak47.ammo);
			}
		}
		world.onceSounds.push({ path: `obstacles/stone_break.mp3`, position: this.body.position });
	}
