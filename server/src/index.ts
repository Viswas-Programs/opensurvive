...
import { Ammo, Bullet, Grenade, GrenadeLoot, Gun, Player } from "./store/entities";
...
server.on("connection", async socket => {
	...
	const f = new GrenadeLoot("frag_grenade", 1)
	f.position = player.position;
	const e = new Grenade(player, 100, Vec2.UNIT_X.addAngle(player.direction.angle()).scaleAll(0.5), 75, 1);
